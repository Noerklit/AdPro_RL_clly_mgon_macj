...
BY CLARA LYNGERAA, MADS NØRKLIT, AND MATHILDE GONZALEZ
...
* Identify inputs the parameters (α, ε, and γ). Note they all range
  ...
  131, 'undiscounted',  we set γ to one. This means that you can simply
  ignore it. The pseudocode in the book seems to forget to declare it.

* A Q-table can be represented in several ways. The easiest is
  probably `Map[S, Map[A, Double]]` where S is the type of states of
  the agent, and A is a type of actions of the agent (`Map` is immutable).
...
* The actions seem to be best modeled as enum: Up, Down, Left, Right
